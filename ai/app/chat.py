...
def build_prompt(sources: list[dict], question: str, history: list[dict] | None = None) -> str:
    ...
    return (
        ...
        "reviewing or scoring a document the user asks about. Ground factual claims about the "
        "documents in the context and cite the sources you use by their bracketed number, like "
        "[1]. You can also use the conversation so far to answer follow-up questions and to "
        "recall what was said earlier in the chat — for example when the user refers back to a "
        "previous question or answer. Say you don't have enough information only when the answer "
        "is in neither the documents nor the conversation, and then briefly say what is missing."
